...
import { Button, Checkbox, Form, Input, Space, Image, Select } from 'antd';
...
const BuySellGetStartedHoneyBee: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      ...
      if (res && res.data) {
        ...
        const usersWithReferral = res.data
          .filter((user: any) => user.referralCode)
          .map((user: any) => {
            return {
              actualEmail: user.email,
        ...
        if (
          urlReferralCode &&
          usersWithReferral.some(
            (user: any) => user.referralCode === urlReferralCode
          )
        ) {
  ...
  const handleClick = () => {
    ...
    let initialReferralCode = (
      usersLite?.find(
        (user: any) => user?.actualEmail === 'bz@azooca.com'
      ) as any
    )?.referralCode!;
    setSelectedReferralCode(initialReferralCode);
      ...
      <div className="d-inline-flex flex-direction-column  flex-align-center responsive_container">
        <div className="row" style={{ gap: '150px' }}>
          <div className="col d-flex flex-direction-column text-center justify-center flex-align-center">
            ...
            <div
              className="bs_container bs_form card text-center justify-center"
              style={{ maxWidth: `${isMobile ? '440px' : '650px'}` }}
            >
              ...
              {isDataFetched && (
                ...
                >
                          ...
                          return (
                            ...
                            >
                              <LightTooltip
                                title={
                                  <a
                                    href={`${baseHiveURL}/captainbee/${user.username}`}
                                    target="_blank"
                                    style={{
                                      textTransform: 'none',
                                      color: 'var(--main_body)',
                  ...
                  >
                    Don’t know any Hive Captain?
                    <div className="hive_link" onClick={handleClick}>
                      {' '}
                      Click here and the company will help assign a Hive Captain
                      for you.
                    </div>
                  </div>
