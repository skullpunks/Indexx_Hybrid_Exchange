  ...
  <Menu
    ...
    items={[
      { key: 2, label: <Link to="/indexx-exchange/buy-sell">Buy & Sell</Link> },
      {
        key: 1,
        label: (
          <Link to="/indexx-exchange/coming-soon?page=decentralized">
            Decentralized
          </Link>
        ),
      },
      {
        key: 3,
        label: (
          <a href="https://tokens.indexx.ai/" target="_blank" rel="noreferrer">
            indexx Tokens
          </a>
        ),
      },
...
const menuEarn = (
  <Menu
    ...
    items={[
      {
        key: 1,
        label: <Link to="/indexx-exchange/trade-to-earn">Trade to earn</Link>,
      },
      {
        key: 2,
        label: <Link to="/indexx-exchange/coming-soon?page=Farms">Farms</Link>,
      },
      {
        key: 3,
        label: <Link to="/indexx-exchange/coming-soon?page=Pools">Pools</Link>,
      },
    ]}
  />
);
  ...
  <Menu
    ...
    items={[
      { key: 1, label: <Link to="/indexx-exchange/help">About</Link> },
      {
        key: 2,
        label: (
          <Link to="/indexx-exchange/coming-soon?page=Hybrid Exchange">
            Hybrid Exchange
          </Link>
        ),
      },
...
const Header = () => {
                ...
                <li className="nav-item">
                  ...
                  >
                    <Dropdown overlay={menuProducts} trigger={["click"]}>
                      <Link
                        to=""
                        className="text-white font_18x padding-r-1_x nav-item"
                        onClick={(e) => e.preventDefault()}
                      >
                        <Space className="nav-link dropdown-toggle">
                          Products
                        </Space>{" "}
                      </Link>
                    </Dropdown>
                  ...
                  >
                    <Dropdown overlay={menuEarn} trigger={["click"]}>
                      <Link
                        ...
                        onClick={(e) => e.preventDefault()}
                      >
                        <Space className="nav-link dropdown-toggle">Earn</Space>{" "}
                      </Link>
                    </Dropdown>
                  </a>
                </li>
    ...
    <Menu
      ...
      items={[
        { key: 1, label: <Link to="/indexx-exchange/about">Dashboard</Link> },
        {
          key: 2,
          label: (
            <Link to="/indexx-exchange/hybrid-exchange">
              Account & Settings
            </Link>
          ),
        },
        ...
        // { key: 5, label: <Link to="/indexx-exchange/blog">Export Reports </Link> },
        {
          key: 5,
          label: (
            <Link to="/indexx-exchange/buy-sell/task-center">Task Center</Link>
          ),
        },
        {
          key: 6,
          label: <Link to="/indexx-exchange/buy-sell/wallet">Wallet</Link>,
        },
        {
          key: 7,
          label: (
            <Link to="/indexx-exchange/buy-sell/withdraw-crypto">Withdraw</Link>
          ),
        },
        {
          key: 8,
          label: (
            <Link to="/indexx-exchange/buy-sell/deposit-crypto">Deposit</Link>
          ),
        },
        {
          key: 9,
          label: (
            <Link to="/indexx-exchange/buy-sell/transaction-history">
              Transaction History
            </Link>
          ),
        },
  ...
  const showText: any = Links.filter((link) =>
    location.pathname.includes(link.value)
  ).map((obj) => obj.label);
        ...
        <div className="link-container">
          <div className="d-flex flex-align-center">
            <h1 className="logo mb-0 d-flex">
              ...
              location.pathname.includes("login") ? (
                ...
                <a href={showUrl[0]} className="logo__text">
                  {showText[0]}
                </a>
              )}
            </h1>
          ...
          {location.pathname.includes("help") && (
            <Button
              ...
              shape="round"
              size="large"
              className="btn_xl header_connect_wallet"
              ...
              {localStorage.getItem("user") ? (
                <>
                  <Button
                    ...
                    shape="round"
                    size="large"
                    className="btn_xl buy_sell_button margin-l-3x buy_crypto_btn"
                    onClick={() => navigate("/indexx-exchange/buy-sell/")}
                  >
                    Buy Crypto
                  </Button>
                  <BellOutlined
                    className="padding-l-2x"
                    style={{ fontSize: 20 }}
                  />
                  <Dropdown overlay={userMenu} trigger={["click"]}>
                    <Link
                      to=""
                      onClick={(e) => e.preventDefault()}
                      className="padding-l-1x text-white"
                    >
                      <Space>
                        {" "}
                ...
                <>
                  ...
                  <Button
                    ...
                    shape="round"
                    size="large"
                    className="btn_xl buy_sell_button margin-l-3x"
                    onClick={() =>
                      navigate("/indexx-exchange/buy-sell/get-started")
                    }
                  >
